...
const getLanguageLinks = async (config, page, contentType, defaultURL) => {
    ...
    await Promise.all(page.localizations.map(async (translation) => {
        ...
        if (!config.contentTypes[contentType]['languages'][locale]
            && config.contentTypes[contentType]['languages']['und']) {
            locale = 'und';
        }
        else if (!config.contentTypes[contentType]['languages'][locale]
            && !config.contentTypes[contentType]['languages']['und']) {
            return null;
        }
...
const createSitemap = async (cache, invalidationObject) => {
    ...
    try {
        const { sitemapEntries, cacheEntries, } = await getService('core').createSitemapEntries(invalidationObject);
